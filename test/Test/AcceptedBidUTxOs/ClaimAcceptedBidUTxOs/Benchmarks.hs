...
module Test.AcceptedBidUTxOs.ClaimAcceptedBidUTxOs.Benchmarks where
...
-- Benchmarks
-------------------------------------------------
-- | Claim multiple valid Bid UTxOs. The bids are for three NFTs and uses three assets
-- for the bid.
...
-- | Claim multiple valid Bid UTxOs, each one for different nfts. The Auction UTxOs each have 
...
benchTest2 number = do
  ...
  -- Try to create the ClaimBid UTxO.
