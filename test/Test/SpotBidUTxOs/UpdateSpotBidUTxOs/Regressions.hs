...
module Test.SpotBidUTxOs.UpdateSpotBidUTxOs.Regressions where
...
regressionTest1 = do
  let -- Seller Info
      ...
      -- Bid Info
      newSpotBidDatum = unsafeCreateSpotBidDatum $ NewSpotBidInfo
        { nftPolicyId = testTokenSymbol
        , bidderCredential = bidderCred
        , nftNames = 
            [ "TestToken1" ]
  ...
  void $ transact bidderPersonalAddr [aftermarketAddress,refScriptAddress] [bidderPayPrivKey]
    emptyTxParams
      ...
      , outputs = flip map [newSpotBidDatum] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
          Output
            { outputAddress = aftermarketAddress
            , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                [ PV2.singleton beaconCurrencySymbol "Bid" 1
...
regressionTest2 = do
  let -- Seller Info
      ...
      newSpotBidDatum1 = bidDatum1 & #bidDeposit .~ 6_000_000
      newSpotBidDatum2 = bidDatum2 & #bidDeposit .~ 6_000_000
      newSpotBidDatum3 = bidDatum3 & #bidDeposit .~ 6_000_000
  ...
  void $ transact bidderPersonalAddr [aftermarketAddress,refScriptAddress] [bidderPayPrivKey]
    emptyTxParams
      ...
      , outputs = flip concatMap [newSpotBidDatum1,newSpotBidDatum2,newSpotBidDatum3] $ 
          \datum@SpotBidDatum{bid=Prices bid,..} ->
            [ Output
                { outputAddress = aftermarketAddress
                , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                    [ PV2.singleton beaconCurrencySymbol "Bid" 1
...
regressionTest3 = do
  ...
  void $ transact 
    ...
    emptyTxParams
      { tokens =
          [ TokenMint
              { mintTokens = flip concatMap [bidDatum1] $ \SpotBidDatum{..} ->
                  [ ("Bid", 1)
                  , (unPolicyBeacon $ toPolicyBeacon nftPolicyId, 1)
                  , (unBidderId $ toBidderId bidderCredential, 1)
                  ]
      ...
      , outputs = flip concatMap [bidDatum1] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
          [ Output
              { outputAddress = aftermarketAddress
              , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                  [ PV2.singleton beaconCurrencySymbol "Bid" 1
  ...
  void $ 
    ...
    emptyTxParams
      { tokens =
          ...
          , TokenMint
              { mintTokens = flip concatMap [bidDatum2] $ \SpotBidDatum{..} ->
                  [ ("Bid", 1)
                  , (unPolicyBeacon $ toPolicyBeacon nftPolicyId, 1)
                  , (unBidderId $ toBidderId bidderCredential, 1)
                  ]
      ...
      , outputs = flip concatMap [bidDatum2] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
          [ Output
              { outputAddress = aftermarketAddress
              , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                  [ PV2.singleton beaconCurrencySymbol "Bid" 1
...
regressionTest4 = do
  ...
  void $ transact bidderPersonalAddr [refScriptAddress] [bidderPayPrivKey] $
    emptyTxParams
      ...
      , outputs = mconcat
          [ flip concatMap [bidDatum1] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
              [ Output
          ...
          , flip concatMap [bidDatum2] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
              [ Output
  ...
  void $ transact 
    ...
    emptyTxParams
      ...
      , outputs = mconcat
          [ flip concatMap [newSpotBidDatum1] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
              [ Output
                  { outputAddress = aftermarketAddress1
                  , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                      [ PV2.singleton beaconCurrencySymbol "Bid" 1
          ...
          , flip concatMap [newSpotBidDatum2] $ \datum@SpotBidDatum{bid=Prices bid,..} ->
              [ Output
                  { outputAddress = aftermarketAddress2
                  , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                      [ PV2.singleton beaconCurrencySymbol "Bid" 1
