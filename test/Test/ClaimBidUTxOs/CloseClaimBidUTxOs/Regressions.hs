...
module Test.ClaimBidUTxOs.CloseClaimBidUTxOs.Regressions where
...
-------------------------------------------------
-- | Close a single valid ClaimBid UTxO. The bid is for only one NFT. The bid uses ada.
...
regressionTest1 = do
  ...
  void $ transact bidderPersonalAddr [aftermarketAddress,refScriptAddress] [bidderPayPrivKey]
    emptyTxParams
      { tokens =
          [ TokenMint
              { mintTokens = flip concatMap bidUTxOs $ \_ ->
                  [ ("Bid", -1)
                  , (unPolicyBeacon $ toPolicyBeacon nftPolicyId, -1)
                  , (unBidderId $ toBidderId bidderCredential, -1)
                  ]
...
-- | Close multiple valid ClaimBid UTxOs, each one for different nfts. The ClaimBid UTxOs each have 
...
-- | Close multiple valid ClaimBid UTxOs that are located at different addresses. 
