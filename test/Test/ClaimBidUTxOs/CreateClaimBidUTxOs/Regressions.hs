module Test.ClaimBidUTxOs.CreateClaimBidUTxOs.Regressions where
...
-------------------------------------------------
-- | Create a single valid ClaimBid UTxO. The bid is for only one NFT. The bid uses ada.
...
-- | Create a single valid ClaimBid UTxO. The bid is for only one NFT. The bid does not use ada.
...
-- | Create a single valid ClaimBid UTxO. The bid is for three NFTs. The bid uses ada and native
-- assets.
...
-- | Create multiple valid ClaimBid UTxOs, each one for different nfts. The ClaimBid UTxOs each have 
...
-- | Create multiple valid ClaimBid UTxOs that are located at different addresses. 
...
regressionTest6 = do
  ...
  void $ transact bidderPersonalAddr [refScriptAddress] [bidderPayPrivKey] $
    emptyTxParams
      ...
      , outputs = mconcat
          [ flip map [bidDatum1] $ \datum@ClaimBidDatum{..} ->
              Output
          ...
          , flip map [bidDatum2] $ \datum@ClaimBidDatum{..} ->
              Output
...
-- | Create a single valid ClaimBid UTxO. The bid is for only one NFT. The bid uses ada. The bid
-- expires.
...
regressionTest7 = do
  let -- Seller Info
      ...
      bidDatum@ClaimBidDatum{..} = unsafeCreateClaimBidDatum $ NewClaimBidInfo
        ...
        , nftNames = 
            [ "TestToken1" ]
        , bidExpiration = Just $ slotToPosixTime 1200
        , claimExpiration = slotToPosixTime 3600
        , bidDeposit = 5_000_000
        , bid = 
            [ (Asset (adaSymbol,adaToken), 20_000_000) ]
