module Test.ClaimBidUTxOs.CreateClaimBidUTxOs.Benchmarks where
...
benchTest1 number = do
  let -- Seller Info
      ...
      -- Bid Info
      bidDatums = flip map (grouped 3 assets) $ \xs -> unsafeCreateClaimBidDatum $ NewClaimBidInfo
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all benchmark scenarios for creating ClaimBid UTxOs.
tests :: [TestTree]
tests =
  [ mustSucceed "benchTest1" $ benchTest1 27

  , mustExceedTxLimits "perfIncreaseTest1" $ benchTest1 28
  ]
