module Test.BidUTxOs.CreateBidUTxOs.Benchmarks where
...
benchTest1 number = do
  let -- Seller Info
      ...
      -- Bid Info
      bidDatums = flip map (grouped 3 assets) $ \xs -> unsafeCreateBidDatum $ NewBidInfo
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all benchmark scenarios for creating Bid UTxOs.
tests :: [TestTree]
tests =
