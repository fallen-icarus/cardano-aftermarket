module Test.BidUTxOs.CreateBidUTxOs.Failures where
...
valueFailure1 = do
  let -- Seller Info
      ...
      -- Bid Info
      bidDatum@BidDatum{..} = unsafeCreateBidDatum $ NewBidInfo
        { nftPolicyId = testTokenSymbol
        , bidderCredential = bidderCred
        , nftNames = 
            [ "TestToken1" ]
...
-- Datum Failures
-------------------------------------------------
-- | The UTxO does not have a BidDatum.
...
datumFailure3 = do
  let -- Seller Info
      ...
      -- Bid Info
      BidDatum{bid=Prices bid,..} = unsafeCreateBidDatum $ NewBidInfo
        { nftPolicyId = testTokenSymbol
        , bidderCredential = bidderCred
        , nftNames = 
            [ "TestToken1" ]
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all failure scenarios for creating Bid UTxOs.
tests :: [TestTree]
tests =
  [ -- Beacon Failures
