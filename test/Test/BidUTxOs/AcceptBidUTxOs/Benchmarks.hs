...
module Test.BidUTxOs.AcceptBidUTxOs.Benchmarks where
...
benchTest1 number = do
  let -- Seller Info
      ...
      -- Bid Info
      datums = flip map (grouped 3 assets) $ \xs -> unsafeCreateBidDatum $ NewBidInfo
  ...
  forM_ (grouped 15 datums) $ \bidDatums ->
    ...
    [bidderPayPrivKey]
    emptyTxParams
      { tokens = flip map bidDatums $ \BidDatum{..} ->
          TokenMint
            { mintTokens = 
                [ ("Bid", 1)
      ...
      , outputs = flip map bidDatums $ \bidDatum@BidDatum{bid=Prices bid,..} ->
          Output
  ...
  bidUTxOs <- take number <$> txOutRefsAndDatumsAtAddress @BidDatum aftermarketAddress
...
benchTest3 number = do
  let -- Seller Info
      ...
      -- Bid Info
      bidDatums = flip map (grouped 3 nfts) $ \xs -> unsafeCreateBidDatum $ NewBidInfo
        { nftPolicyId = testTokenSymbol
        , bidderCredential = bidderCred
        , nftNames = sort xs
        , paymentAddress = toPlutusAddress bidderPersonalAddr
  ...
  bidUTxOs <- take number . filter (isJust . snd) <$> 
    txOutRefsAndDatumsAtAddress @BidDatum aftermarketAddress
