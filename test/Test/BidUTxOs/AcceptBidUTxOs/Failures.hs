...
module Test.BidUTxOs.AcceptBidUTxOs.Failures where
...
beaconFailure7 = do
  ...
  void $ transact sellerPersonalAddr [aftermarketAddress,refScriptAddress] [sellerPayPrivKey]
    emptyTxParams
      { tokens = flip map (take 1 bidUTxOs) $ \(_, Just BidDatum{..}) -> 
          TokenMint
            { mintTokens = 
                [ ("Bid", -1)
                , (unPolicyBeacon $ toPolicyBeacon nftPolicyId, -1)
...
beaconFailure8 = do
  ...
  void $ transact bidderPersonalAddr [refScriptAddress] [bidderPayPrivKey] $
    emptyTxParams
      { tokens = [ ]
      , outputs = flip map [bidDatum] $ \datum@BidDatum{} ->
          Output
            { outputAddress = aftermarketAddress
...
paymentDatumFailure2 = do
  ...
  void $ transact sellerPersonalAddr [aftermarketAddress,refScriptAddress] [sellerPayPrivKey]
    emptyTxParams
      ...
      , outputs = flip map bidUTxOs $ \(_, Just BidDatum{..}) ->
          Output
            { outputAddress = toCardanoApiAddress paymentAddress
            , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                [ mconcat $ flip map nftNames $ \name -> 
...
paymentAddressFailure2 = do
  ...
  void $ transact 
    ...
    emptyTxParams
      ...
      , outputs = flip concatMap [bidDatum1,bidDatum2] $ \datum@BidDatum{bid=Prices bid,..} ->
          [ Output
              { outputAddress = aftermarketAddress
              , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                  [ PV2.singleton beaconCurrencySymbol "Bid" 1
  ...
  void $ transact sellerPersonalAddr [aftermarketAddress,refScriptAddress] [sellerPayPrivKey]
    emptyTxParams
      ...
      , outputs = flip map (zip [(1::Int)..] bidUTxOs) $ \(i,(ref, Just BidDatum{..})) ->
          if odd i then
            Output
              { outputAddress = sellerPersonalAddr
              , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
  ...
  void $ transact sellerPersonalAddr [aftermarketAddress,refScriptAddress] [sellerPayPrivKey]
    emptyTxParams
      ...
      , outputs = intersperse extraOutput $
          flip map (reverse bidUTxOs) $ \(ref, Just BidDatum{..}) ->
            Output
              { outputAddress = toCardanoApiAddress paymentAddress
              , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                  [ mconcat $ flip map nftNames $ \name -> 
  ...
  void $ transact sellerPersonalAddr [aftermarketAddress,refScriptAddress] [sellerPayPrivKey]
    emptyTxParams
      ...
      , outputs = intersperse extraOutput $ mconcat
          [ [extraOutput]
          , flip map (take 1 bidUTxOs) $ \(ref, Just BidDatum{..}) ->
              Output
                { outputAddress = toCardanoApiAddress paymentAddress
                , outputValue = utxoValue (fromIntegral $ bidDeposit - 1) $ mconcat
                    [ mconcat $ flip map nftNames $ \name -> 
          ...
          , flip map (drop 1 bidUTxOs) $ \(ref, Just BidDatum{..}) ->
              Output
                { outputAddress = toCardanoApiAddress paymentAddress
                , outputValue = utxoValue (fromIntegral bidDeposit) $ mconcat
                    [ mconcat $ flip map nftNames $ \name -> 
...
tests =
  ...
  , scriptMustFailWithError "beaconFailure8"
      "Invalid Bid UTxO found"
      beaconFailure8

    -- Payment Value Failures
  , scriptMustFailWithError "paymentValueFailure1"
      "Auction payment UTxO has wrong value"
  ...
  , scriptMustFailWithError "paymentDatumFailure1"
      "Not all required auction payments found"
  ...
  , scriptMustFailWithError "inputTypeFailure1"
      "UTxO is not an Auction UTxO or a Bid UTxO"
      inputTypeFailure1
  ]
